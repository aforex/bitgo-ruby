module Bitgo
  module V2
    ...
    class Api
      ...
      def initialize(end_point, currency)
        @end_point = end_point
        @currency = currency
      end
      ...
      # TODO: update to api v2 if necessary
      ...
      # TODO: update all label methods to api v2
      # def list_labels
      #   call :get, '/labels'
      # end
      #
      ...
      def add_wallet(label:, m:, n:, keys:, enterprise: nil, isCold: nil, disableTransactionNotifications: nil)
        wallet_params = { label: label, m: m, n: n, keys: keys }
        wallet_params[:enterprise] = enterprise if enterprise.present?
        wallet_params[:isCold] = isCold if isCold.present?
        wallet_params[:disableTransactionNotifications] = disableTransactionNotifications if disableTransactionNotifications.present?


        call :post, "/#{@currency}/wallet", wallet_params
      end
